...
- Type validation against the XRPL type system
- JSON output
...
- Parameter order
- Parameter flags (required/optional)
- Return types

Without an ABI, developers would need to manually maintain this information separately from the code.

## Automatic Generation

Bedrock automatically generates the ABI file (`abi.json`) during `bedrock deploy`. You don't need to run a separate command.
...
Bedrock rejects invalid types with helpful error messages during `deploy`.
...
/// @param resolver ACCOUNT - Resolver address
/// @flag 1
...
**Recommended:** Commit `abi.json` to version control so reviewers can see ABI changes in PRs and deployment scripts can rely on a committed ABI.

## Troubleshooting

### "invalid type 'X' for parameter 'Y'"

Using a type that's not in the XRPL type system. Common mistakes:
- `STRING` -> Use `VL`
- `ADDRESS` -> Use `ACCOUNT`
...
- Ensure you're using `///` (doc comments), not `//`
- Check that annotations are directly above the function
- Verify you're in a bedrock project directory

### "duplicate parameter name"

Each parameter must have a unique name within a function.
